...
from pymritools.utils import nifti_save, nifti_load, torch_load, root_sum_of_squares, fft_to_img, ifft_to_k
...
def smap(settings: CoilSensConfig):
    ...
    for idx_z in tqdm.trange(data.shape[2], desc="Batch Processing"):
        ...
        if not settings.input_in_img_space:
            batch_data = torch.abs(ifft_to_k(batch_data, dims=(0, 1)))
        # do rsos
        ...
        bd_smap_k = fft_to_img(batch_data_smap, dims=(0, 1))
        nxh = int(nx / 2)
        nyh = int(ny / 2)
        ...
        bd_smap_k[:, :nyh-kh] = 0
        bd_smap_k[:, nyh+kh:] = 0
        bd_smap_smoothed = torch.abs(ifft_to_k(bd_smap_k, dims=(0, 1)))
