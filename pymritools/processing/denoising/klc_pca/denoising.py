...
def denoise(k_space: torch.Tensor, noise_scans: torch.Tensor,
    ...
    # assume we have dims (num_noise_scans, num_samples, n_channels)
    ...
    # input assumes (nr, np, ns, nc, ne) - take batch num to be echoes
    noise_scans = torch.movedim(noise_scans, 0, -1)
    log_module.info(f"\t\tget singular values")
    _, noise_s_vals = denoise_data(
        k_space=noise_scans[:, None, None, :, :], device=device, batch_size=50
    )
...
def denoise_data(k_space: torch.Tensor, line_patch_size: int = 0,
    ...
    log_module.info(f"Assume 5D data (nr, np, ns, nc, ne). Found input shape: {k_space.shape}.")
    ...
    n_lines = n_lines[0]
    mask = mask.unsqueeze(0).unsqueeze(2).unsqueeze(2).expand((nr, np, ns, nc, ne))
    sampled_k_lines = k_space[mask]

    logging.info(f"denoising")
    # move batch dims to front and channels to back (nr, np, ns, nc, ne) -> (np, ns, ne, nr, nc)
    ...
    # allocate
    s_vals = torch.zeros((num_batches, m))
    ...
    if ev_noise is None and ev_weighting_fn is None:
        ...
        ev_weighting_fn = torch.ones_like(ev_noise)

    for i in tqdm.trange(num_batches, desc="Batch processing"):
        ...
        s_vals[batch_start:batch_end] = s.cpu()
