...
def main():
    ...
    # setup logging
    print(f"logging to: {path_out.joinpath('log.txt').as_posix()}")
    logging.basicConfig(
    ...
    df_losses = pl.DataFrame({
        "loss": losses, "sar": losses_snr, "snr": losses_sar, "run": torch.arange(idx + 1).tolist(),
        # "corr": losses_corr
    })
    fas = torch.tensor(fas)

    fig = go.Figure()
    for i, idf in enumerate([
        "loss", "snr", "sar",
        # "corr"
    ]):
        fig.add_trace(
