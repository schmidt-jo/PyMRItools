...
from pymritools.utils import torch_save, ifft_to_k, root_sum_of_squares, nifti_save, HidePrints
import twixtools
...
def pulseq_rd_to_torch(config: RD):
    ...
    if config.visualize:
        ...
        for i in tqdm.trange(img.shape[2], desc="FFT"):
            img[:, :, i] = ifft_to_k(k_space[:, :, i], dims=(0, 1))
        if config.debug:
...
def siemens_rd_to_torch(config: RD):
    ...
    if config.visualize:
        # transform into image
        img = ifft_to_k(k_space, dims=(0, 1))
        # do rSoS
