...
def get_linear_indices_mod(
        k_space_shape: tuple[int, ...],
        patch_shape: tuple[int, ...],
        sample_directions: tuple[int, ...],
        flip_directions: tuple[int, ...] = None) -> tuple[torch.Tensor, tuple[int, ...]]:
    """Computes linear indices for multidimensional k-space sampling.
    ...
    patch_shape = torch.tensor(patch_shape).flip(0)
    sample_directions = torch.tensor(sample_directions).flip(0)
    if flip_directions is None:
        flip_directions = (0,) * len(sample_directions)
    flip_directions = torch.tensor(flip_directions).flip(0)
    ...
    if not torch.all((flip_directions == 0) | (flip_directions == 1)):
        raise ValueError("Sample directions must be 0 or 1")
    ...
    patch_sizes = torch.where(patch_shape == 0, torch.tensor(1), patch_shape)
    patch_sizes = torch.where(patch_shape == -1, k_space_shape, patch_sizes)
    # Create indices for patches (invert if flip_directions[i] == 1)
    patch_steps = [
        (torch.arange(size) if flip == 0 else torch.arange(size - 1, -1, -1))
        for size, flip in zip(patch_sizes, flip_directions)
    ]
    ...
    # so that the patch never gets outside the bounds.
    sample_steps = torch.where(sample_directions == 0, torch.tensor(1), k_space_shape - patch_sizes + 1)
    # Create sample indices (also consider flipping directions)
    sample_steps = [
        (torch.arange(size) if flip == 0 else torch.arange(size - 1, -1, -1))
        for size, flip in zip(sample_steps, flip_directions)
    ]
    ...
    # Combine patch and sample indices
    indices = patch_indices.unsqueeze(0) + sample_indices.unsqueeze(1)
    return indices.flatten(), (*indices.shape,)
