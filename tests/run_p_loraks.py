...
def run_p_loraks():
    """
    Runs an example recon and plots results
    """
    import logging
    logging.basicConfig(level=logging.INFO)
    loraks = (PLoraks()
              .with_max_iterations(50)
              .with_regularization_lambda(0.6)
              .with_c_matrix()
              .with_lowrank_algorithm(LowRankAlgorithmType.TORCH_LOWRANK_SVD, q=30, niter=2)
              .with_sv_hard_cutoff(25)
    ...
    jupiter_k_subsampled = jupiter.sub_sample_ac_random_lines(acceleration, 40).to(dtype=torch.complex64)
    ...
    output_dir = get_test_result_output_dir("p_loraks_quality", mode = ResultMode.VISUAL)
    fig.write_html(os.path.join(output_dir, "p_loraks_reconstruction.html"))
